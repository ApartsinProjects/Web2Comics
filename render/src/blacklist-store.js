...
class FileBlacklistStore {
  constructor(options = {}) {
    this.filePath = path.resolve(String(options.filePath || 'render/data/blacklist.json'));
  }
...
function createBlacklistStoreFromEnv() {
  ...
  return {
    ...
    impl: new FileBlacklistStore({
      filePath: process.env.RENDER_BOT_BLACKLIST_FILE || 'render/data/blacklist.json'
    })
  };
}
