...
class FileKnownUsersStore {
  constructor(options = {}) {
    this.filePath = path.resolve(String(options.filePath || 'render/data/known-users.json'));
  }
...
function createKnownUsersStoreFromEnv() {
  ...
  return {
    ...
    impl: new FileKnownUsersStore({
      filePath: process.env.RENDER_BOT_KNOWN_USERS_FILE || 'render/data/known-users.json'
    })
  };
}
