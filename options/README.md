...
## Purpose
Extension Options page UI and settings management, provider configuration, prompts, and storage tools.
...
- `options.js` : Options controller for loading/saving settings, provider credential validation state, provider model test actions (`TEST_PROVIDER_MODEL`), prompt-template validation/persistence, storage maintenance actions, and local recommended model default import (`shared/recommended-model-set.local.json`).
