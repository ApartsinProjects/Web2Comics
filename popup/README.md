...
## Contents
- `popup.html` : Popup UI markup for the launcher (`Create Comic` / `See History`), generator wizard form, progress panel, custom-style modal, and popup history modal. Includes small help (`?`) links into the user manual.
...
- `popup.js` : Popup controller logic for content extraction, provider/model selection, wizard readiness and hard-guarding, generation start/progress polling, ETA strings, provider filtering by configuration/validation, custom style management, history modal behavior, and sidepanel auto-open.
